...
String cityCode = "101040100";  //天气城市代码查询地址 https://www.it610.com/article/1291702105907732480.htm
int tempnum = 0;   //温度百分比
int huminum = 0;   //湿度百分比
int tempcol = 0xffff;
int humicol = 0xffff;
...
int prevTime = 0;
int AprevTime = 0;
#define BL_addr 1 //被写入数据的 EEPROM 地址编号  0亮度
#define CC_addr 10 //被写入数据的 EEPROM 地址编号  10城市
  ...
  if (Serial.available() > 0)
  {
    ...
    if (SMOD == "0x01") //设置1亮度设置
    {
      ...
      if (LCDBL >= 0 && LCDBL <= 100)
      {
        ...
        delay(5);
        int LCD_BL_PWM = EEPROM.read(BL_addr);
        delay(5);
        SMOD = "";
        Serial.printf("亮度调整为：");
    ...
    if (SMOD == "0x02") //设置2地址设置
    {
      ...
      if (CityC >= 101000000 && CityC <= 102000000)
      {
...
void myclock_setup()
{
  ...
  getCityWeater();  // 将时间设为60 以便后续loop刷新
  Hour_sign   = 60;
  Minute_sign = 60;
  Second_sign = 60;
}

static time_t prevDisplay = 0; // 获取的时间
